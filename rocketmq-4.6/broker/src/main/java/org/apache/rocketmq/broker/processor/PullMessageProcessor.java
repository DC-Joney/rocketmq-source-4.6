...
public class PullMessageProcessor implements NettyRequestProcessor {
    ...
    private RemotingCommand processRequest(final Channel channel, RemotingCommand request, boolean brokerAllowSuspend)
        ...
        //解析、处理：requestHeader中的sysFlag
        // 1 没有消息时Broker是否阻塞？
        // 2 Consumer是否上报了CommitOffsetFlag提交标志？
        // 3 是否需要根据子表达式过滤消息？

        // 没有消息时, 是否要求broker阻塞等待
        final boolean hasSuspendFlag = PullSysFlag.hasSuspendFlag(requestHeader.getSysFlag());
        // 是否上报了CommitOffsetFlag提交标志
        ...
        if (hasSubscriptionFlag) {
            try {
                ...
                // 如果不是tag过滤，而是通过SQL92语法过滤
                // 构建 ConsumerFilterData对象
                // SqlFilter, SQL92的方式
        ...
        //Broker会根据filterSupportRetry属性, 创建不同的MessageFilter 实现类, 包装前面的  consumerFilterData
        MessageFilter messageFilter;
        if (this.brokerController.getBrokerConfig().isFilterSupportRetry()) {
            //ExpressionForRetryMessageFilter支持重试队列里的消息过滤
        ...
        //一切准备就绪，通过MessageStore检索消息，主要流程如下：
        //定位到ConsumeQueue
        //校验拉取的Offset
        ...
        if (getMessageResult != null) {
            ...
            //nextBeginOffset告诉Consumer下次拉取的位点
        ...
        // 存储ConsumerOffset
        // 在响应结果前，对Consumer上报的消费offset进行存储：
