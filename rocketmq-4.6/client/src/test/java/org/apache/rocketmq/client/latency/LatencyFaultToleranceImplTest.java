...
public class LatencyFaultToleranceImplTest {
    ...
    @Test
    public void cTestUpdateFaultItem() throws Exception {
        latencyFaultTolerance.updateFaultItem("BrokerA", 3000, 3000);
        assertThat(latencyFaultTolerance.isAvailable("BrokerB")).isTrue();
        assertThat(latencyFaultTolerance.isAvailable("BrokerA")).isFalse();

        MQFaultStrategy strategy = new MQFaultStrategy();

        long currentLatency = 550L;
        long notAvailableDuration = strategy.computeNotAvailableDuration(currentLatency);

        latencyFaultTolerance.updateFaultItem("BrokerC", currentLatency, notAvailableDuration);
        boolean isAvailable = latencyFaultTolerance.isAvailable("BrokerC");
        System.out.println("notAvailableDuration = " + notAvailableDuration + " isAvailable = " + isAvailable);


        currentLatency = 530L;
        notAvailableDuration = strategy.computeNotAvailableDuration(currentLatency);

        latencyFaultTolerance.updateFaultItem("BrokerC", currentLatency, notAvailableDuration);
        isAvailable = latencyFaultTolerance.isAvailable("BrokerC");
        System.out.println("notAvailableDuration = " + notAvailableDuration + " isAvailable = " + isAvailable);
