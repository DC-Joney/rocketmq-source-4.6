...
public class PullSysFlag {
    private final static int FLAG_COMMIT_OFFSET = 0x1;  //是否上报了CommitOffset提交偏移
    private final static int FLAG_SUSPEND = 0x1 << 1;    // 没有消息时, broker是否阻塞等待
    private final static int FLAG_SUBSCRIPTION = 0x1 << 2;  // 是否需要根据子表达式过滤消息
    private final static int FLAG_CLASS_FILTER = 0x1 << 3;  //  是否  classFilter
