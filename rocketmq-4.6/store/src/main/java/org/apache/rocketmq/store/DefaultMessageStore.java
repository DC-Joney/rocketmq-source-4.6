...
public class DefaultMessageStore implements MessageStore {
    ...
    public GetMessageResult getMessage(final String group, final String topic, final int queueId, final long offset,
        ...
        //一个Topic下可以有多个队列，每个队列对应一个ConsumeQueue，
        // 根据Topic和queueId定位ConsumeQueue
            ...
            if (maxOffset == 0) {
                ...
                if (bufferConsumeQueue != null) {
                    try {
                        ...
                        int i = 0;
                        // 高性能策略：减少过滤的消息数量
                        // 消息过滤、消息检索会导致CommitLog文件随机读，通过葵花宝典可知道， 随机读的降低性能
                        // 所以, Broker会限制单次过滤的最大消息数，最大是800 （16000/20=800）
                        final int maxFilterMessageCount = Math.max(16000, maxMsgNums * ConsumeQueue.CQ_STORE_UNIT_SIZE);
                        final boolean diskFallRecorded = this.messageStoreConfig.isDiskFallRecorded();
                        ConsumeQueueExt.CqExtUnit cqExtUnit = new ConsumeQueueExt.CqExtUnit();
                        for (; i < bufferConsumeQueue.getSize() && i < maxFilterMessageCount; i += ConsumeQueue.CQ_STORE_UNIT_SIZE) {

                            // 消息在CommitLog中的offsetPy物理偏移量
                            ...
                            // 如果 messageFilter 不为空，需要过滤
                            // 先根据TagsHash快速匹配: 判断tagsCode是否存在SubscriptionData的codeSet集合内
                            // 如果不是，则直接略过
                            // 否则，校验通过
                            ...
                            //根据offsetPy偏移量/物理长度，去读取CommitLog的文件映射缓冲区
                            ...
                            //所有的消息过滤都通过后，就会将结果追加到 GetMessageResult
...
//                    }
    ...
    public void handleScheduleMessageService(final BrokerRole brokerRole) {
        if (this.scheduleMessageService != null) {
            //从节点，关闭  scheduleMessageService
            if (brokerRole == BrokerRole.SLAVE) {
                this.scheduleMessageService.shutdown();

            } else {
                //从节点，开始 scheduleMessageService
